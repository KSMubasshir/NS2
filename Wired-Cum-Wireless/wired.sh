...
awk -f wired_awk.awk wired-and-wireless.tr > wired.out
...
thrput_file="throughputWired.out"
delay_file="delayWired.out"
delRatio_file="deliveryRatioWired.out"
dropRatio_file="dropRatioWired.out"

echo -ne "$(($r*10)) 	$thr\n" >> $thrput_file
echo "$(($r*10)) 	$del" >> $delay_file
...
echo -ne "$(($r*10))$del_ratio\n" >> $delRatio_file
echo "$(($r*10)) 	$dr_ratio" >> $dropRatio_file

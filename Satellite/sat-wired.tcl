...
for {set i 0} {$i < [expr $numOfNodes]} {incr i} {
	set n_($i) [$ns node]
}
...
set angDist [ expr 360/$numOfNodes ] 
for {set i 0} {$i < [expr $numOfNodes]} {incr i} {
	$n_($i) set-position $opt(alt) $opt(inc) 0 [expr 0 + ( $i * $angDist ) ] $plane
}
...
for {set i 0} {$i < [expr $numOfNodes]} {incr i} {
	set nodeNo [expr  ( $i + 1 )  % $numOfNodes ]
	$n_($nodeNo) set_next $n_($i);
}
...
$n100 add-gsl geo $opt(ll) $opt(ifq) $opt(qlim) $opt(mac) $opt(bw_up) \
  $opt(phy) [$n111 set downlink_] [$n111 set uplink_]
...
$n200 add-gsl polar $opt(ll) $opt(ifq) $opt(qlim) $opt(mac) $opt(bw_up) \
  $opt(phy) [$n_(5) set downlink_] [$n_(5) set uplink_]
...
for {set i 0} {$i < [expr $numOfNodes]} {incr i} {
	#$n_(expr[($i+1)%$numOfNodes)]) set_next $n_($i);
	$ns add-isl intraplane $n_($i) $n_([expr ($i+1) % $numOfNodes]) $opt(bw_isl) $opt(ifq) $opt(qlim)
}

###########################################################################
# Set up wired nodes                                                      #
###########################################################################
# Connect $n300 <-> $n301 <-> $n302 <-> $n100 <-> $n111 <-> $n101 <-> $n303
#                      ^                   ^
#                      |___________________|    
#
